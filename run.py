...
def report():
	if 'OpenID' in session:
		...
		if len(d) == 0:
			...
			Popen('sh ' + FILE_PREFIX + 'history_basic.sh ' + session['OpenID'] + ' ' + APPID + ' ' + session['AccessToken'] + ' ' + str(1180627200) + ' ' + session['Username'] + ' ' + FILE_PREFIX, shell=True)
			...
			if int(s) == 0:
				if int(time.time()) > int(timestamp) + 3600 * 24:
					...
					Popen('sh ' + FILE_PREFIX + 'history_basic.sh ' + session['OpenID'] + ' ' + APPID + ' ' + session['AccessToken'] + ' ' + str(int(timestamp) - 600) + ' ' + session['Username'] + ' ' + FILE_PREFIX, shell=True)
